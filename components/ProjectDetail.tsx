...
export function ProjectDetail({ project }: { project: Project }) {
  ...
  return (
    <main className="inner-page">
      <Navbar />
      <header className="project-header"><p className="eyebrow">Selected work · {project.brand}</p><h1>{project.title}</h1><div className="project-meta"><p><span>Category</span>{project.category}</p><p><span>Location</span>{project.location}</p><p><span>Year</span>{project.year}</p></div></header>
      <section className="project-intro"><p>{project.description}</p></section>
