    ...
    <main className="inner-page">
      ...
      <section className="project-list" aria-label="Project list">
        {projects.map((project, index) => <article className="project-card" key={project.slug}>
          ...
          <Link className={`project-cover project-cover-${project.slug}`} href={`/projects/${project.slug}`}><Image unoptimized src={project.coverImage} width={1200} height={1500} sizes="(max-width: 767px) 100vw, (max-width: 1023px) 50vw, 33vw" alt={`${project.brand} completed project interior`} /></Link>
          <div className="project-summary"><h2>{project.brand}</h2><p className="card-category">{project.category}</p><dl><div><dt>Location</dt><dd>{project.location}</dd></div><div><dt>Scope</dt><dd>Commercial Interior</dd></div></dl></div>
